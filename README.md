...
| 脚本 | 用途 | 入口 |
| --- | --- | --- |
| `_render_claret.py` | 导入 `克拉蕾.pmx`，自动三点布光、构图、曝光校准，输出三视图 | `blender.exe -b --factory-startup --python _render_claret.py` |
| `克拉蕾_control.py` | 克拉蕾 行走 / 跳跃控制脚本（键盘实时控制 + 关键帧烘焙） | 见 [克拉蕾_control使用说明.md](克拉蕾_control使用说明.md) |
